...
class RNN(nn.Module):
    def __init__(self):
        ...
        self.rnn = nn.LSTM(input_size=len(x_train_tensor[0]), 
            hidden_size=128, num_layers=2, batch_first=True, dropout=0.5)
...
for epoch in range(200):
    ...
    for step, (x, b_label) in enumerate(data_loader):
        b_x = x.view(BS, -1, len(x_train_tensor[0]))
        b_y = b_label
...
for each in data_scaler.rev_y_validation:
    y_fore_validation2.append(each[0][0])
